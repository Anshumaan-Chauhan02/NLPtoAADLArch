...
stopwords_remove=['from','to','and','it','with','form']
#Different energy types 
energy_list=['Human','Acoustic','Biologcal','Chemical','Electrical','ElectroMagnetic','Hydraulic','Magnetic','Mechanical','Thermal','Radioactive','Pneumatic']
#List of words present in Functional basis
separate_list=['separate','divide','extract','remove','isolate','sever','disjoin','detach','detaches','release','sort','split','disconnect','subtract','refine',
               'filter','purify','purifies','percolate','strain','clear','cut','drill','lathe','polish','polishes','sand']
...
transfer_list=['transfer','transport','transmit','carry','carries','deliver','advance','lift','move','conduct','convey']
guide_list=['guide','translate','rotate','allow','direct','shift','steer','straighten','switch','switches','move','relocate','spin','turn','constrain',
            'unfasten','unlock']
...
regulate_list=['regulate','increase','decrease','control','equalize','limit','maintain','open','close','delay','interrupt']
change_list=['change','increment','decrement','shape','condition','adjust','modulate','demodulate','invert','normalize','rectify',
             'rectifies','reset','scale','vary','varies','modify','modifies','amplify','amplifies','enhance','magnify', 'magnifies',
             'multiply','multiplies','attenuate','dampen','reduce','compact','compress','compresses','crush','crushes','pierce','deform','form','prepare','adapt','treat']
stop_list=['stop','prevent','inhibit','end','hault','pause','interrupt','restrain','disable','turn off','shield','insulate','protect','resist']
convert_list=['convert','condense','create','decode','differentiate','digitize','encode','evaporate','generate','integrate','liquefy','liquifies','process','solidify',
...
sense_list=['sense','detect','measure','feel','determine','discern','perceive','recognize','identify','identifies','locate']
indicate_list=['indicate','track','display','announce','show','denote','record','register','mark','time','expose','select']
...
temperature=['Cryogenic','Frozen','Chilled','Freezing','Cold','Cool','Nomral Temperature','Room Temperature','Lukewarm','Toasty', 'Mild', 'Warm',
...
#Adding some words to the stop words list that we do not require
stopwords_add=['The','A']
...
#input_paragraph="The xyz inputs water from abc."
...
for i in new_para.split("."):
  ...
  elif (("converts" in i) or ("mixes" in i) or ("couples" in i) or ("separates" in i) or ("energizes" in i) or ("deenergizes" in i) or ("stores" in i) or ("stops" in i) or ("changes" in i) or ("regulates" in i)):
    within_words=i.split()
    words=['converts','mixes','couples','separates','energizes','deenergizes','stores','stops','changes','regulates']
    for i in words:
      if i in within_words:
        occured_word=i
...
for i in str.split("."):
  ...
  for j in within_words:
    if (j.isupper() or (j in functional_verbs) or (j in temperature) or (j in velocity) or (j in material) or (j in additional_reservde_words) or (j in stopwords_remove)):
      continue
    else:
